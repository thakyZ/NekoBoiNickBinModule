...
Try {
	...
	Write-Warning -Message "Failed to read Audio Repeater Settings from `"$((Join-Path -Path $PSScriptRoot -ChildPath "config.json"))`". Trying `"$((Join-Path -Path $PSScriptRoot -ChildPath "AutoAudioRepeater.cfg"))`" instead.";
	Try {
		$ConfigFile = $Null;
