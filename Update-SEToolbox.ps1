...
Begin {
  [string]$RepositoryOwner = "mmusu3";
  [string]$RepositoryName = "SEToolbox";
  [string]$Repository = "$($RepositoryOwner)/$($RepositoryName)";

  [string]$Releases = "https://api.github.com/repos/$($Repository)/releases";

  [string]$DownloadDest = (Join-Path -Path 'D:' -ChildPath 'Modding' -AdditionalChildPath @('Tools', 'SpaceEngineers', 'SEToolbox'));
